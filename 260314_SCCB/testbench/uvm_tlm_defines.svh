...
`define UVM_SEQ_ITEM_PULL_IMP(imp, REQ, RSP, req_arg, rsp_arg) \
  ...
  function void put_response(input RSP rsp_arg); \
       `UVM_IF_METHOD_BEGIN(rsp_arg,"put_response") \
       imp.put_response(rsp_arg); \
       `UVM_IF_METHOD_END(rsp_arg,"put_response") \
  endfunction \
  task get(output REQ req_arg); \
       `UVM_IF_METHOD_BEGIN(req_arg,"get") \
       imp.get(req_arg); \
       `UVM_IF_METHOD_END(req_arg,"get") \
  endtask \
  task peek(output REQ req_arg); \
       `UVM_IF_METHOD_BEGIN(req_arg,"peek") \
       imp.peek(req_arg); \
       `UVM_IF_METHOD_END(req_arg,"peek") \
  endtask \
  task put(input RSP rsp_arg); \
       `UVM_IF_METHOD_BEGIN(rsp_arg,"put") \
       imp.put(rsp_arg); \
       `UVM_IF_METHOD_END(rsp_arg,"put") \
  endtask
// End
`else
`define UVM_SEQ_ITEM_PULL_IMP(imp, REQ, RSP, req_arg, rsp_arg) \
